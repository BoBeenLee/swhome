...
<%@ page
	import="data.bo.ArticleBO,data.bo.FileBO,data.vo.article.PartArticle,data.vo.file.FileInfo"%>

<%!private int selectModifyPartArticle(HttpServletRequest request) {
	...
	private void procNoticeDate(HttpServletRequest request, Date noticeDate) {
		...
		request.setAttribute("noticeMonth", calendar.get(Calendar.MONTH));
...
<%
	...
	// Setting
	selectModifyPartArticle(request);
...
<link rel="stylesheet" type="text/css"
	href="/skhusw/common/css/article.css" />
...
<script type="text/javascript"
	src="/skhusw/common/js/board/plupload.full.js"></script>
...
<body>
	<div class="rightContent article">
		<form id="partForm" action="/skhusw/UpdateServlet" method="post">
			<table>
				<tr>
					<th colspan="1">제목</th>
					<th colspan="5"><input name="title" type="text"
						style="width: 100%;" value="${ partArticle.partArticleTitle }">
					</th>
				</tr>
				<tr>
					<th>공지여부</th>
					<th><input type="checkbox" name="notice"
						<c:if test="${ partArticle.noticeYn == 'y' }">checked='checked'</c:if>>
					</th>
					<th>공지기간</th>
					<th style="text-align: left !important;"><select
						name="noticeYear">
							<c:forEach var="year" begin="<%= calendar.get(Calendar.YEAR) %>"
								end="<%= calendar.get(Calendar.YEAR) + 200 %>" step="1">
			...
			<div class="articleContent">
				<textarea id="articleContent" name="articleContent" rows="30"
					class="tinymce">
				${ partArticle.partArticleContent }
			</textarea>
			</div>
			<div id="articleMenu">
				<!--  파일업로드 부분 -->
				<div id="fileContainer">
					...
					<br /> <span id="fileCount">0</span> <a id="pickfiles"
						href="javascript:void(0)">[Select files]</a>
				</div>
				<div>
					<ul>
						<c:forEach var="filetmp" items="${ fileList }">
							<li style="list-style: none;"><span>${
									filetmp.fileName }</span> <a href="javascript:void(0)"
								onclick="deleteFile('${ filetmp.fileId }')">X</a></li>
						</c:forEach>
					</ul>
				</div>
				<!--  파일 업로드 하는 부분 -->
																					...
																					$(
																							...
																							.append(
																									'<div id="' + file.id + '" >'
																											...
																											+ ') <a href="javascript:void(0)">X</a> <b></b>'
				...
				<!--  파일 업로드 하는 부분 -->
				<input id="partSubmit" type="button" value="확인"> <input
					type="hidden" name="idx" value="${ idx }" /><input type="hidden"
					name="idx" value="${ idx }" /><input type="hidden"
					name="articleId" value="${ partArticle.partArticleId }"><input
