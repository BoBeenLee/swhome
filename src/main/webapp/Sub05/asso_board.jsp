...
<script type="text/javascript">
	jQuery(document).ready(
			function($) {
				...
				$("#assoSearch").click(
						function(e) {
							window.document.location.href = "#asso?req=1&"
									+ "searchType="
...
<body>
	<div>
		<img src="Sub05/img/content-1/subtop5-1.PNG" width="754px" />
	</div>
	<div class="rightContent">
		<div id="asso_container" class="bbs">
			<table>
				<thead>
					<tr>
						<th width="60px">번호</th>
				...
				<tbody id="asso_records">
					<c:forEach items="${ articleList }" var="article" varStatus="num">
						<tr>
							<td id="${ article.norArticleId }"><c:if
									test="${ article.noticeYn == 'y' }">Notice</c:if>
								<c:if test="${ article.noticeYn == 'n' }">${articleSize - num.count
								+ 1}</c:if></td>
							<td style="text-align: left !important;"><a
								href="#asso?req=1&articleId=${ article.norArticleId }&type=read">${
									article.norArticleTitle } [${ article.commentCount }]</a></td>
							<td>${ article.userName }</td>
							<td><%=new Date(((NorArticle) pageContext
						.getAttribute("article")).getNorArticleDate().getTime())%></td>
							<td>${ article.hit }</td>
						</tr>
					</c:forEach>
				</tbody>
		...
		<div align="center">
			...
			<c:if
				test="${ userInfo != null && ( userInfo.codeName == '관리자' || userInfo.codeName == '교수' || userInfo.codeName == '졸업생' ) }">
				<input type="button"
					onclick="javascript_:window.document.location.href='#asso?req=1&type=write'"
					value="글쓰기" />
			</c:if>
		</div>
		<!--! end of #content -->
