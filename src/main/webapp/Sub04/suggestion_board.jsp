	...
	jQuery(document).ready(
			function($) {
				$("div .bbs_list_num").jPages({
					containerID : "suggestion_records",
					previous : "←",
					next : "→",
					perPage : 15,
					delay : 20
				...
				$("#suggestionSearch").click(
						function(e) {
							window.document.location.href = "#board?req=2&"
									+ "searchType="
				...
				$("#searchText").keypress(function(e){
					if ((e.which && e.which == 13) || (e.keyCode && e.keyCode == 13)) {
						$("#suggestionSearch").trigger("click");
						return false;
					} else
						return true;				
				});
...
<body>
	<div>
		<img src="Sub04/img/content-2/subtop4-2.PNG" width="754px" />
	</div>
	<div id="suggestion_content" class="rightContent">
		<div id="suggestion_container" class="bbs">
			<table>
				<thead>
					<tr>
						<th width="60px">번호</th>
					...
					<c:forEach items="${ articleList }" var="article" varStatus="num">
						<tr>
							...
							<td style="text-align: left !important;"><a
								<c:if test="${ article.partPassword == null }">href="#board?req=2&articleId=${ article.partArticleId }&type=read"</c:if>
		...
		<div align="center">
			...
			<c:if test="${ userInfo != null }"><input type="button"
				onclick="javascript_:window.document.location.href='#board?req=2&type=write'"
				value="글쓰기" /></c:if>
		</div>
		<!--! end of #content -->
